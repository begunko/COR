#!/usr/bin/env python3
"""
Универсальный скрипт для сбора кода проекта в один текстовый файл.
Автоматически определяет имя проекта по названию папки.
Исключает автоматически генерируемые файлы, миграции, кэш, базы данных,
медиа-ресурсы (модели, текстуры, шрифты), документацию, тесты,
служебные файлы Git, лицензии, сторонние библиотеки и сам скрипт сбора.
"""
...
def get_project_name():
    """Получает имя проекта из названия текущей папки"""
    current_dir = Path.cwd().resolve()
    return current_dir.name


def collect_project_code(project_path=".", output_file=None):
    """Собирает содержимое всех файлов с кодом в один текстовый файл"""

    # Определяем имя проекта и файл вывода
    project_name = get_project_name()
    if output_file is None:
        output_file = f"{project_name}_code.txt"

    # Исключаем сам скрипт (по любому возможному имени)
    script_name = Path(__file__).name

    # === ИСКЛЮЧАЕМЫЕ ДИРЕКТОРИИ ===
    exclude_dirs = {
        # --- УЖЕ СУЩЕСТВУЮЩИЕ ИСКЛЮЧЕНИЯ ---
        # Python
        ...
        # --- НОВЫЕ ИСКЛЮЧЕНИЯ ДЛЯ ПРОЕКТА THRE.JS и подобных ---
        # Тестовые данные, модели, звуки, текстуры, шрифты
        "models",            # 3D модели (.gltf, .obj, и т.д.)
        "sounds",            # Аудиофайлы
        "textures",          # Текстурные карты (.png, .jpg, .exr...)
        "fonts",             # Шрифты (.ttf, .woff...)
        "ies",               # Файлы фотометрии для света (IES)
        "luts",              # Таблицы цветокоррекции (LUT)
        "materialx",         # .mtlx файлы описания материалов
        "resources",         # Различные ресурсы (может содержать изображения, шрифты)

        # Документация, руководства, скомпилированные страницы
        "docs",              # Папка с документацией (HTML, JSON)
        "manual",            # Руководства пользователя (HTML)
        "editor",            # Код редактора three.js (отдельный проект, а не ядро)

        # Сторонние библиотеки
        "libs",              # Сторонние библиотеки (ammo, draco, lil-gui и т.д.)

        # Тесты
        "test",              # Папка с unit/e2e тестами
        "tests",             # Альтернативное имя для папки с тестами

        # Утилиты для разработки, а не код библиотеки
        "devtools",          # Инструменты разработчика Chrome
        "utils",             # Скрипты для сборки, changelog, сервер
    }
    ...
    print(f"🔍 Сканирую файлы проекта '{project_name}'...")
    ...
    for root, dirs, files in os.walk(project_path):
        ...
        for file in sorted(files):
            ...
            # Проверка: исключаем сам скрипт
            if file == script_name:
                skipped_count += 1
                skipped_files_list.append(f"{file} (этот скрипт)")
                continue
            ...
            # Проверка: исключаем выходные файлы проекта (старые сборки)
            if file.endswith("_code.txt"):
                skipped_count += 1
                skipped_files_list.append(f"{file} (выходной файл)")
                continue

            # Исключаем файлы, имя которых содержит ключевые слова
            exclude_patterns_in_name = [
                "search.json",     # Индекс поиска для доков
                ".typeface.json",  # Данные шрифтов
                "llms-full.txt",   # Полная LLM-справка (в основном не код)
                "llms.txt",        # Краткая LLM-справка (не код)
                "files.json",      # Индексный файл example-ов
                "tags.json",       # Теги для example-ов
            ]
            should_exclude = False
            for pattern in exclude_patterns_in_name:
                if pattern in file:
                    should_exclude = True
                    skipped_count += 1
                    skipped_files_list.append(f"{file} (по шаблону имени '{pattern}')")
                    break
            if should_exclude:
                continue
            ...
            try:
                ...
                if size > 10 * 1024 * 1024:  # > 10MB
                    print(
                        f"  ⚠️  Пропущен большой файл: {file} ({size // 1024 // 1024}MB)"
                    )
                    skipped_count += 1
                    continue
            except:
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        # Заголовок файла
        out.write("=" * 80 + "\n")
        out.write(f"ПРОЕКТ: {project_name}\n")
        out.write(
            f"Дата создания отчёта: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        )
        out.write(f"Директория проекта: {Path.cwd().resolve()}\n")
    ...
    if output_path.exists():
        print(f"💾 Размер отчёта: {output_path.stat().st_size / 1024:.1f} KB")
...
if __name__ == "__main__":
    project_name = get_project_name()
    print(f"🚀 Начинаю сбор кода проекта '{project_name}'...\n")
